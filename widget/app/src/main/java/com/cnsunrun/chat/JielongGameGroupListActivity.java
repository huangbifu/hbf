 ...
 * 接龙游戏
 */
public class JielongGameGroupListActivity extends LBaseActivity implements PageLimitDelegate.DataProvider{
                ...
                RongIM.getInstance().joinGroup(item.id,item.title, new RongIMClient.OperationCallback() {
                            ...
                            public void onSuccess() {
                                if(!ConversationActivity.isConversationRuning()) {
                                    ...
                                    getSession().put("game_group_type"+item.id,"2");
...
//                    UIUtils.shortM("初始化");
    ...
    public void loadData(int page) {
        if(pageLimitDelegate.page==1) {
            UIUtils.showLoadDialog(that, "加载中..");
            BaseQuestStart.game_packet_get_group(that, page);
