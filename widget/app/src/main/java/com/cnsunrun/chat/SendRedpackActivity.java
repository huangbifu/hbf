...
import static com.cnsunrun.common.quest.BaseQuestConfig.SEND_WELFARE_REDPACK_CODE;
import static com.cnsunrun.login.event.LoginConfig.IS_SECRET_FREE;

/**
 * 发红包
 */
public class SendRedpackActivity extends LBaseActivity {
    ...
    EditText etBombNumber;
    @BindView(R.id.btnSubmit)
    QMUIRoundButton btnSubmit;
    String password;
    UserDataBean userDataBean;
    boolean isSetDealPwd;
    @BindView(R.id.llleiDian)
    LinearLayout llleiDian;
    private int redpack_num;

    public static void startThis(Activity that) {
        Intent intent = new Intent(that, SendRedpackActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        redpack_num = getIntent().getIntExtra("redpack_num", 7);
        if (redpack_num == 100) {
            etRedPackNumber.setEnabled(true);
            llleiDian.setVisibility(View.GONE);
        } else {
            etRedPackNumber.setEnabled(false);
            llleiDian.setVisibility(View.VISIBLE);
...
//                CommonIntent.startModifyPwdActivity(that,"1".equals(isSetDealPwd)?1:2);
    ...
    public void nofityUpdate(int requestCode, BaseBean bean) {
        ...
        switch (requestCode) {
            case SEND_WELFARE_REDPACK_CODE://福利包
...
//
    ...
    private void submitRedPackInfo() {
        ...
        if (redpack_num == 100) {
            BaseQuestStart.sendWelfareRedPack(SendRedpackActivity.this, etMoney, etRedPackNumber, group_id, password);
        } else {
            BaseQuestStart.send_redpack(SendRedpackActivity.this, etMoney, etRedPackNumber, etBombNumber, group_id, password, type, 0);
        }
    }
}
