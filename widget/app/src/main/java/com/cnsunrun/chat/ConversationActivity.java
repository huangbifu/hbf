...
import com.cnsunrun.common.dialog.RecharTipDiglog;
 ...
 * 咵天-聊天页面
 */
public class ConversationActivity extends LBaseActivity implements View.OnClickListener {
    ...
    @BindView(R.id.btnSendRedpack)
    QMUIRoundButton btnSendRedpack;
    @BindView(R.id.btnShowBalance)
    QMUIRoundButton btnShowBalance;
    ...
    public static ConversationActivity tmpConversation;
    private String id;
...
//        isCustom = getSession().getBoolean("customer_id_" + targetId);//标记客服
        ...
        setContentView(R.layout.activity_chat_group_conversation);
        ...
        if (uri != null) {
            ...
            titleNameStr = uri.getQueryParameter("title");
            titleBar.setTitle(titleNameStr);
        ...
        groupType = getSession().getString("game_group_type" + targetId);
        if ("2".equals(groupType)) {
            btnSendRedpack.setVisibility(View.GONE);
        } else {
            btnSendRedpack.setVisibility(View.VISIBLE);
        }
...
//        StatusBarConfig.assistActivity(this);
        ...
        titleBar.setRightAction(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                isOtherPage = true;
                if (isGroup) {
                    CommonIntent.startChatGroupSetInfoActivity(that, targetId);
                } else {
        ...
        if (EmptyDeal.isEmpy(titleNameStr)) {
...
//            rc_edit_text2.setVisibility(View.GONE);
        } else if (isRedpackGroup) {
            findViewById(io.rong.imkit.R.id.rc_emoticon_toggle).setEnabled(false);
            findViewById(io.rong.imkit.R.id.rc_emoticon_toggle).setVisibility(View.GONE);
            ImageView rc_yu_e = (ImageView) findViewById(io.rong.imkit.R.id.rc_yu_e);
            TextView rc_edit_text2 = (TextView) findViewById(io.rong.imkit.R.id.rc_edit_text2);
            rc_redpack = (ImageView) findViewById(io.rong.imkit.R.id.rc_redpack);
            rc_yu_e.setVisibility(View.VISIBLE);
            rc_redpack.setVisibility(View.VISIBLE);

            View.OnClickListener yueListener = new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    getYuEInfo();
                }
            };
            rc_yu_e.setOnClickListener(yueListener);
            View.OnClickListener sendRedpackListener = new View.OnClickListener() {
                @Override
                public void onClick(View view) {

                }
            };
            rc_redpack.setOnClickListener(sendRedpackListener);
            findViewById(R.id.btnSendRedpack).setOnClickListener(this);
            findViewById(R.id.btnShowBalance).setOnClickListener(this);
            findViewById(R.id.btnWelfareSendRedpack).setOnClickListener(this);
            findViewById(R.id.btnNineRedpack).setOnClickListener(this);
        } else {
//            EditText rc_edit_text = (EditText) findViewById(io.rong.imkit.R.id.rc_edit_text);
//            TextView rc_edit_text2 = (TextView) findViewById(io.rong.imkit.R.id.rc_edit_text2);
//            rc_edit_text.setHint("说点什么");
    ...
    protected void onResume() {
        ...
        if (isRedpackGroup) {
            if (!isSendRedpack) {
                ...
                RongIMClient.getInstance().cleanRemoteHistoryMessages(Conversation.ConversationType.GROUP, targetId, 0, new RongIMClient.OperationCallback() {
                    @Override
...
//                    BaseQuestStart.postLogcat(that,"JoinGrop_Success");
            ...
            case NEIGHBORHOOD_IM_GROUP_INFO_CODE:
                if (bean.status == 1) {
                    groupInfoBean = bean.Data();
//                    titleBar.setTitle(groupInfoBean.getTitle());
                    redpack_num = groupInfoBean.redpack_num;
                    member_num = EmptyDeal.size(groupInfoBean.member_list);
                    if (groupInfoBean.isRedpackGroup()) {
                        isRedpackGroup = true;
                        rc_redpack.setVisibility(View.VISIBLE);
                        getSession().put(Config.KEY("redpack_group_id"), targetId);
                        if (!isLoaded) {
                            isLoaded = true;
                            getSession().put("is_redpack" + targetId, true);
                        }
                    } else {
                        rc_redpack.setVisibility(View.GONE);
                    }
...
//
            ...
            case BaseQuestStart.GRAB_REDPACK_CODE:
                ...
                RedPackMessage redPack = (RedPackMessage) rcMessage.getContent();
                if (13 == bean.status) {
                    RecharTipDiglog.newInstance().setOnSubmitAction(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            RechargeActivity.startThis(that);
                        }
                    }).show(getSupportFragmentManager(), "RecharTipDiglog");
...
//                    tvMoney.setText(jobj.optString("money"));
                    ...
                    CommonIntent.startRedpackPageActivity(that, id, targetId, redPack.getType());
                    return;
                }

                RedpacketDialog.newInstance()
                        .setId(id)
                        .setType(redPack.getType())
                        .setStates(bean)
                        .setRcMessage(rcMessage)
                        .show(getSupportFragmentManager(), "RedpacketDialog");
...
//        if (!needShowYueTip) return;
    ...
    public void onMessageClick(UIMessage message) {
        ...
        if (content instanceof RedPackMessage) {
            ...
            if (extra.startsWith(Const.REDPACK_GETED) && !extra.contains("13")) {//不是余额不足
                isOtherPage = true;
                CommonIntent.startRedpackPageActivity(that, redPack.getId(), targetId, redPack.getType());
                return;
            }
            id = redPack.getId();
            showRedpacketDialog(id, redPack.getType());
        }
    }

    private void showRedpacketDialog(String id, String type) {
        UIUtils.showLoadDialog(that, "领取中..");
        BaseQuestStart.grab_redpack(this, id, type);
    }
    ...
    private void loadPageData() {
        ...
        if (!isGroup) {
            ...
            BaseQuestStart.NeighborhoodImGroupInfo(that, targetId);
        }
        refreshChatBg();
    }
    ...
    protected void onDestroy() {
        ...
        if (isGroup && isRedpackGroup) {
            getSession().remove(Config.KEY("redpack_group_id"));
            ...
            RongIMClient.getInstance().quitGroup(targetId, new RongIMClient.OperationCallback() {
                @Override
                public void onSuccess() {
                    BaseQuestStart.postLogcat(ConversationActivity.this, "quitGrop_success");
                }

                @Override
                public void onError(RongIMClient.ErrorCode errorCode) {
                    BaseQuestStart.postLogcat(ConversationActivity.this, "quitGrop_ErrorCode:" + errorCode);
                }
            });
...
//            });
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btnSendRedpack://七包
                isSendRedpack = true;
                isOtherPage = true;
                CommonIntent.startSendRedpackActivity(that, targetId, member_num, redpack_num, titleNameStr, groupType);
                break;
            case R.id.btnNineRedpack://九包
                isSendRedpack = true;
                isOtherPage = true;
                EventBus.getDefault().postSticky(groupInfoBean);
                CommonIntent.startSendNineRedpackActivity(that, targetId, member_num, 9, titleNameStr, groupType);
                break;
            case R.id.btnWelfareSendRedpack://福利包
                isSendRedpack = true;
                isOtherPage = true;
                CommonIntent.startSendRedpackActivity(that, targetId, member_num, 100, titleNameStr, groupType);
                break;
            case R.id.btnShowBalance://余额
                getYuEInfo();
                break;
        }
    }
