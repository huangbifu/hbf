 ...
 * 重置支付密码
 */
public class ModifyPayPasswordActivity extends LBaseActivity {
    ...
    private void initViews() {
        editNewPassword.setMaxLength(6);
    }
    ...
    public void nofityUpdate(int requestCode, BaseBean bean) {
        switch (requestCode) {
            case BaseQuestConfig.QUEST_GET_MODIFY_PAY_PASSWORD_VERIFICATION_CODE:
                if (bean.status == 1) {
                    //获取短信验证码
//                    ToastUtils.shortToast(bean.Data());
                ...
                break;
            case BaseQuestConfig.QUEST_PAY_PASSWORD_CODE:
                if (bean.status == 1) {
                    //修改
                    ToastUtils.shortToast(bean.msg);
                    AHandler.cancel(task);
        ...
        switch (view.getId()) {
            case R.id.get_verification_code:
                ...
                if (task == null) {
                    ...
                    BaseQuestStart.getModifyPayPasswordVerificationCode(that);
                }
                break;
            case R.id.tv_modify:
                //修改
                ...
                BaseQuestStart.modifyPayPassword(that, editNewPassword.getText().toString(), editVerificationCode.getText().toString());
                break;
        }
    }
